...
console.log("=== deckReveal：公開ゾーン経由で選んでもデッキが余計に減らない ===")
{
    ...
    // **slot は 0 → 1 の順に埋めること**（2026-08-23 修正）。逆順にすると slot=1 へ入れた
    // スピリットを次の slot=0 の探索（i >= 0）が同じ1枚として拾い直し、元の先頭カードを
    // index1 へ押し出してしまう。結果スピリットが1枚しか残らず、上5枚に他のスピリットが
    // 無いときだけ落ちる——揺らぐテストとして残っていた
    {
