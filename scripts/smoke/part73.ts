...
}
{
    ...
    // anySide化により要塞龍ギガ自身（Lv1 BP5000）もdestroyの対象になりうる
    // （非対話時の自動選択は実効BP最大＝同値は相手優先）ため、target はBP5000以上にして
    // 自陣より優先的に選ばれるようにする（リザードマンLv2＝コア2個でBP6000＝境界値ちょうど）
    const target = put(s, "p2", "BS01-018", 2) // リザードマン Lv2 BP6000（6000以下の境界値）
