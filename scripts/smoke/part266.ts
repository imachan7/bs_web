...
//
// 2026-09-26の統合で coresDownToLimit は sequence + removeCores(downTo) に置き換わった
// （実データは data/cards/BS10.json BS10-019-e2）。選択待ちの形も options 固定式から
// removeCores 共通の候補id方式（スピリットはinstanceId、ゾーンは `__reserve__<pid>` 等）に変わった。
//
...
// removeCores（target:"spread"／downTo）のゾーン候補id（server/src/logic/actions/removeCores.ts sourceIdOf）
const reserveIdOf = (pid: PlayerId): string => `__reserve__${pid}`
const trashIdOf = (pid: PlayerId): string => `__trash__${pid}`
function downToLimitAction(limit: number): {
    type: "sequence"
    actions: { type: "removeCores"; side: "opponent" | "own"; from: ("spirit" | "nexus" | "reserve" | "trash")[]; to: "void"; count: 0; downTo: number; chooser: "owner" }[]
} {
    return {
        type: "sequence",
        actions: (["opponent", "own"] as const).map((side) => ({
            type: "removeCores",
            side,
...
console.log("=== 非対話：相手→自分の順に、それぞれ合計5個以下になる ===")
{
    ...
    resolveAction(s, "p1", src, downToLimitAction(5))
    ...
    // 2026-09-26の統合でremoveCoresの非対話自動順は共通のZONE_ORDER（相手が選ぶ側はその逆順）になり、
    // 「相手が選ぶ」側はトラッシュ→リザーブ→ネクサス→スピリットの順になった
    assert(s.players.p2.trashCores === 0 && s.players.p2.reserve === 1, "自動順はトラッシュ→リザーブから先に取る")
...
console.log("=== 対話：取り先は1個ずつ**コアを失う側**が選ぶ（相手→自分で選択者が入れ替わる） ===")
{
    ...
    const vanillaP2 = s.players.p2.field.spirits[0]!.instanceId
    assert(s.pendingChoice?.kind === "target", "removeCores共通の候補id方式（ゾーンはid・個体はinstanceId）")
    ...
    assert(
        cands.includes(reserveIdOf("p2")) && cands.includes(trashIdOf("p2")) && cands.includes(vanillaP2),
        "ゾーンとフィールドの個体が候補に出る",
    )

    assert(act(s, "p1", { type: "resolveChoice", instanceId: reserveIdOf("p2") }) !== null, "発生源の持ち主は1段目を選べない")

    // p2 は 12 → 5 なので7回選ぶ。毎回フィールドの個体を優先して選んでも構わない
    let guard = 0
    while (s.pendingChoice?.pid === "p2" && guard++ < 20) {
        const choices = s.pendingChoice?.candidates ?? []
        const pick = choices.includes(vanillaP2) ? vanillaP2 : choices[0]!
        assert(act(s, "p2", { type: "resolveChoice", instanceId: pick }) === null, "p2が取り先を選べる")
    }
    ...
    while (s.pendingChoice !== null && guard++ < 20) {
        const choices = s.pendingChoice?.candidates ?? []
        assert(act(s, "p1", { type: "resolveChoice", instanceId: choices[0]! }) === null, "p1が取り先を選べる")
    }
