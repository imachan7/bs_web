...
export type EffectAction =
    ...
    | { type: "recoverSpiritFromTrash"; costSacrificeChosen?: true; count: number; familyFilter?: FamilyFilter; all?: true; thenDestroyIfFamily?: { family: FamilyFilter; maxBp: number }; costDestroyOwnKeyword?: Keyword; keywordFilter?: Keyword; colorFilter?: Color; nameIncludes?: string; costSkipDraw?: true; includeBraves?: true; vanillaFilter?: true; bravesOnly?: true; costFilter?: { max?: number; min?: number } } // costFilter指定時はカードのコストがこの範囲のものだけ（BS11-051 イビル・フィッシャー＝コスト6以下）
    ...
    | { type: "designateAttackTarget"; pick: "maxBp" } // 相手のフィールドで最もBPの高いスピリット1体を指定する（BS12-008グランド・ドラグキャッスル）。指定はBattleState.designatedBlockerInstanceIdを立てるだけで、実際のブロック確定は「アタック時効果と【バースト】の解決がすべて終わってブロック宣言に入る時点」（GameEngine.doPassのflashCount>=2分岐）で行う。指定時点でboardResistanceAgainst（【装甲】/【重装甲】）を通し、耐性を持つ個体は候補から除外する（候補が居なければ指定しない＝通常のアタック）。同BPが複数ならinteractiveTargetsで選ばせ、非対話はフィールド先頭。指定先が場を離れた／条件を満たさなくなった場合はブロック確定時に通常のアタックへ戻る（アタック宣言後のフラッシュタイミングは消さない）
...
export interface BattleState {
    ...
    designatedBlockerInstanceId?: string // designateAttackTarget が立てる「指定した相手スピリット」。directedと違いブロック宣言自体は通常どおり成立させる必要があるため、GameEngine.doPassのフラッシュ①終了時点（blockerInstanceId未確定）で自動的にfinishBlockDeclarationへ渡す。無効なら（場を離れた等）通常のブロック待ちに戻す。BS12-008
