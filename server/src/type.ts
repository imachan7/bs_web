...
export type EffectAction =
    ...
    | { type: "destroy"; filter?: TargetFilter; count: number; countPerOpponentTrashMagicColors?: boolean; anySide?: true; excludeTarget?: true; chooserIsTarget?: true; drawPerDestroyed?: true } // drawPerDestroyed指定時は、**実際に破壊できた**1体につき自分が1枚ドローする（「この効果で相手のスピリットを破壊したとき、自分はデッキから1枚ドローする」＝BS11-006獅龍皇子レオグルス）。数え方は destroyAll.drawPerDestroyed と同じバッチ経路（「破壊されるかわりにフィールドに残る」で残った個体は数に入らない）
    ...
    | { type: "voidCoreToSelf"; count: number } // ボイドからコアcount個をこのスピリット上に置く（selfがnullならno-op。selfがネクサスのときも同じくネクサス上に置く＝placeCoresOnSpiritはCardInstance汎用。BS02-080等で既に実績あり）
    ...
    | { type: "recoverSpiritFromTrash"; costSacrificeChosen?: true; count: number; familyFilter?: FamilyFilter; all?: true; thenDestroyIfFamily?: { family: FamilyFilter; maxBp: number }; costDestroyOwnKeyword?: Keyword; keywordFilter?: Keyword; colorFilter?: Color; nameIncludes?: string; costSkipDraw?: true; includeBraves?: true; vanillaFilter?: true; bravesOnly?: true; maxCost?: number } // maxCost指定時はそのコスト以下のカードのみ対象（BS11-051イビル・フィッシャー＝コスト6以下）
    ...
    | { type: "costDiscardHandTypeThenCoreRemove"; cardTypes: ("spirit" | "brave")[]; count: number } // 「自分の手札にあるスピリットカード1枚かブレイヴカード1枚を破棄することで、相手のスピリットのコアcount個を相手のリザーブに置く」（BS11-075トーテンタンツ）。COST_MODEL.md §1：破棄できる手札が無ければ**不発**。interactiveTargets時は破棄する手札を選ばせ（kind:"card"）、非対話は末尾の該当カードを破棄する決定的簡略化
    | { type: "costDestroyOwnThenOpponentDestroysToCost"; needCost?: number } // 「自分のスピリット1体を破壊することで、相手は、コスト合計がその破壊したスピリットのコスト以上になるように、相手のスピリットを好きなだけ破壊する」（BS11-076シェアリングペイン）。needCostは**内部専用**（コストにした自分のスピリットのコストを、相手の選択の再入に持ち回る。cards.jsonには書かない）。**選ぶのは相手**（CHOOSER_RULES.md §1）で、対話では1体ずつ選ばせる（coresDownToLimit と同じ形）。コストにする自分の1体は、対話では自分が選び、非対話は実効BP最小を選ぶ決定的簡略化。破壊できる自分のスピリットがいなければ不発
    ...
    | { type: "summonSequence"; byFushi?: true; bySoku?: true } // byFushi指定時は【不死】による召喚として「自分のスピリットが召喚されたとき」を発火する（fieldEvent.fushiSummonOnly の判定に使う）。// 召喚が済んだ後の処理（召喚時効果 →「自分のスピリットが召喚されたとき」誘発 → 天使長ファニムの疲労付与）を self に対して行う。**cards.jsonには書かない内部専用**：【転召】の対象選択で中断したときに、GameEngine が pendingChoice.queue へ積んで選択の解決後に合流させるためだけに使う
    ...
    | { type: "freeMagicAfterPaidMagic"; timesPerTurn: number } // 自分が**コストを支払って**マジックカードを使用したとき、その効果発揮後、自分の手札/手元にあるマジックカード1枚をコストを支払わずに使用できる（BS11-X05魔導双神ジェミナイズLv2-3＝ターンに2回）。非対話（テスト・AI）は使わない＝不発の決定的簡略化
    ...
    | { type: "banBlockByCostsThisTurn"; costs: number[] } // このターンの間、**相手の**指定コスト（完全一致。配列＝OR）のスピリットすべてはブロックできない（BS11-057バタホルン＝コスト4/6/8）。banActByCostThisTurn（アタックもブロックも不可・コスト以下）とは別枠
    ...
    | { type: "reviveLastDestroyedNexus"; coreCost?: number; costFrom?: "ownFieldOrReserve"; colorFilter?: Color } // colorFilter指定時は、直近に破壊されたネクサスがその色のときだけ戻せる（BS11-066発見されし世界樹Lv1＝「自分の**緑の**ネクサスが破壊されたとき」） // costFrom:"ownFieldOrReserve" 指定時は、コストを self 上ではなく**自分のフィールド/リザーブ**のコアから払う（リザーブ優先。SD02-014 魔法監視塔Lv1＝コア1個をトラッシュへ） // self上のコアをコストぶん自分のトラッシュに置くことで、直近に破壊された自分のネクサス（GameState.lastDestroyedNexus）をトラッシュから自分のフィールドへ戻す（coreCost省略時はself上のコアすべて＝BS04戦闘獣ジャッカー。指定時はその数だけ支払う。コア不足なら不発。BS05ブロンズ・ゴレム＝1個）
    ...
    | { type: "opponentHandLockExceptColorThisTurn"; color: Color } // このターンの間、**相手は**指定色以外の手札のカードを使えない（BS11-082ウィッグバインド＝黄以外）。turnConstraints に handLockExceptColorForPid を積み、召喚・配置・マジック使用の検証が見る
    ...
    | { type: "revealAndSummonAllByFamily"; count: number; familyFilter: FamilyFilter; countFromSelfLevel?: true; maxCost?: number } // countFromSelfLevel指定時はcountを無視し**発生源の現在のLv**と同じ枚数をオープンする（BS11-007輝龍皇ヘリオスドラゴン＝「このスピリットのLvと同じ枚数」。selfが無ければno-op）。maxCost指定時は召喚できるのがそのコスト以下のカードだけになる（同＝コスト7以下） // 自分のデッキ上からcount枚を公開し、その中の指定系統（配列＝OR）を持つスピリットカード**すべて**を、コストを支払わず、【転召】させずに召喚する（維持コアが足りない分は召喚できずトラッシュへ。revealAndSummonKeywordと異なり任意選択を挟まない範囲効果）。この効果で召喚されたスピリットの『召喚時』効果は発揮されない（revealAndSummonKeywordは発揮する点と対照的）。系統不一致・召喚できなかったカードはすべてトラッシュへ破棄する（BS08魔帝龍騎ダーク・クリムゾン：上7枚から系統「龍帝」/「竜騎」すべて）
    | { type: "millUntilCostThenSummonFree"; costs: number[]; maxCount: number } // 自分のデッキを上から maxCount 枚を上限に、指定コストのいずれかを持つスピリットカードが出るまでトラッシュへ破棄し、出たらその1枚を**コストを支払わずに**召喚する（維持コアはリザーブから。払えなければ召喚できずトラッシュに残る）。この召喚では『このスピリットの召喚時』効果は発揮されない（BS11-038天星馬ペガシーダ＝コスト6/7・上限6枚）
    ...
    | { type: "revealTopThenFreeUseOrHand"; use: "magicFlash" | "summonSpiritOrBrave" } // 自分のデッキを上から1枚オープンし、use:"magicFlash" ならそれがマジックカードのとき**フラッシュ効果をコストを支払わずに即時に使用できる**（BS11-058神弓鳥ペリュトーン）／use:"summonSpiritOrBrave" ならスピリットカード/ブレイヴカードのとき**コストを支払わずに召喚する**（BS11-X05魔導双神ジェミナイズ。ブレイヴは合体先を選ばずスピリット状態で出す簡略化）。条件に合わない・使わない場合はそのカードを手札に加える
...
export type ConstraintDef =
    | { type: "blockRequiresCorePayment"; count: number } // 相手は、相手のリザーブのコアをこの数だけ相手のトラッシュに置かなければ**このスピリットを**ブロックできない（BS11-037ヒポグリフィーLv2-3）。ブロック宣言の検証で払えるかを見て、宣言時に支払う
...
export type GlobalConstraintDef =
    ...
    | { type: "ownNexusIndestructible"; colors?: Color[]; sourceColors?: Color[]; sourceTypes?: CardType[]; requireAllOwnNexusesColor?: Color } // requireAllOwnNexusesColor指定時は、**発生源の持ち主のネクサスすべてがその色のときだけ**有効（BS11-069黄金の鐘楼＝「自分のネクサスすべてが黄の間」） // colors指定時は、そのいずれかの色を持つネクサスだけを守る（BS09-062ノルンの泉Lv2＝白/黄）。// 発生源の持ち主のネクサスすべては、相手の効果によって破壊されない。
...
export type EffectDef =
    ...
    | {
          // **発生源自身のコストを継続で増減する**（BS11-017ムシャツバメ Lv2-3＝
          // 『自分のアタックステップ』「このスピリットをコスト+3する」）。
          // refreshLevelAsOverrides が CardInstance.costDeltaContinuous に反映し、instCostDelta が読む。
          // 「コスト◯以下を破壊」などコストを見る判定すべてに一度で効く
          id: string
          kind: "selfCostMod"
          levels: number[] | null
          amount: number
    ...
    | {
          ...
          // 合体の唯一の入口 attachBrave が、相手側にこのエントリを有効に持つ発生源があるかを見て疲労させる
          id: string
          kind: "opponentCombineExhaust"
          levels: number[] | null
      }
    | {
          // **トラッシュにあるこのカードを、条件に合う自分のスピリットが召喚されたとき、無償で召喚できる**
          // （BS11-004プロミネンスワイバーン＝「カード名に『太陽』と入っている自分のスピリットが召喚されたとき」）。
          // 【不死】（keyword:"fushi"）と同型だが、**コストは支払わず**、ステップの限定も無い。
          // 破壊処理の途中で聞けない【不死】と同じ理由で、確認は GameState.pendingTrashSummons に積み、
          // アクションを解決しきった安全な地点（GameEngine の requestPendingTrashSummon）で1件ずつ出す
          id: string
          kind: "trashSummonOnOwnSummon"
          levels: null // トラッシュにあるカードの効果なのでレベルは無い（必ず null）
          nameIncludes: string // 召喚されたスピリットのカード名がこの文字列を含むときだけ発揮する
      }
    | {
          // **自分のスピリットが破壊されたとき、そのスピリットを指定コストとしても扱う**
          // （BS11-064 闇の聖剣 Lv1「自分のスピリットが破壊されたとき、そのスピリットをコスト3/4のスピリットとしても扱う」）。
          // 元のコストに**加えて**これらのコストでも扱う＝【不死】の引き金（triggerCosts）の判定が広がる
          id: string
          kind: "destroyedCostAs"
          levels: number[] | null
          costs: number[]
      }
    | {
    ...
    | {
          ...
          battleRole?: "attacker" | "blocker" // 勝利／バトル終了したときの自分の役割がこれと一致する場合のみ発火（省略時は従来通り常に発火）。onBattleWin と onBattleEnd で使える（BS11-007輝龍皇ヘリオスドラゴン＝『このスピリットの**アタック時**』…バトル終了時）
          condition?:
              ...
              | { targetCombined: true } // バトルの相手（fireTriggerのtargetInstanceId）が**合体スピリット**のときのみ発火（instIsCombinedで判定。BS11-X02滅神星龍ダークヴルム・ノヴァLv2-3＝「相手の合体スピリットとバトルしたとき」）
              ...
              | { bothFieldsHaveSpiritMinBp: number } // **お互いのフィールド**に実効BPがこれ以上のスピリットがそれぞれ1体以上いるときのみ発火（BS11-008爆竜ドラゴニックベアード＝BP10000以上）
              | { byOpponentSpiritEffect: true } // **相手のスピリットの効果で**破壊されたときのみ発火（onDestroy用。DestroyContext の sourcePid/sourceType で判定。BS11-034星馬コルット）
    ...
    | {
          ...
          winnerCombinedOnly?: true // 勝利したスピリットが**合体スピリット**（ブレイヴが合体している）ときのみ発火（instIsCombinedで判定。BS11-062オールトの竜巣Lv2＝「自分の合体スピリットが、BPを比べ〜」）
    ...
    | {
          ...
          sokuSummonOnly?: true // event:"ownSpiritSummoned" 限定：その召喚が**【神速】による召喚**だったときのみ発火（BS11-065満天の牧草地Lv2）。fushiSummonOnly の兄弟
    ...
    | {
          ...
          condition?: { ownNexusAtLeast: number } | { ownLifeAtMost: number } // scope:"self"用：発生源の持ち主（＝このカードを使おうとしているプレイヤー）のネクサス数がこれ以上（BS10-059＝1以上）／ライフがこれ以下（BS11-X03星騎士ハーキュリーΩ＝3以下）のときのみ有効
      }
    | {
          id: string
          kind: "activated"
    ...
    | {
          ...
          cost?: {
              sourceCoresToTrash?: number // **発生源（このネクサス／スピリット）の上のコア**をこの数だけ持ち主のトラッシュへ。足りなければ支払い不可＝不発（BS11-066発見されし世界樹Lv2＝コア3個）
    ...
    | {
          id: string
          kind: "reductionGrant"
          zone?: "hand" | "trash" // 軽減シンボルを与える先のゾーン（省略時は "hand"＝従来どおり手札のカード）。"trash" はトラッシュのカードに与える（BS11-013グラシャハウンド＝「自分のトラッシュにある【不死】を持つスピリットカードすべてに軽減シンボル[紫]」。【不死】の召喚コスト計算だけが見る） // 発生源が場にありレベル有効の間、条件成立時に対象カード種別/色の使用コストへ軽減シンボルを付与する（ペンタン／天使バーチュ）
          levels: number[] | null
    ...
    | {
          ...
          levels: number[] | null
          target: "ownAll" | "self" // "self" は**発生源自身だけ**（BS11-039天使ティアエル＝「このスピリットのシンボルを黄のシンボル3つにする」）
          familyFilter?: FamilyFilter
          count: number
    ...
    | {
          ...
          levels: number[] | null
          target: "ownAll" | "opponentAll" // "opponentAll"＝発生源の持ち主から見た**相手の**スピリットすべて（BS11-082ウィッグバインド）
          ...
          nonVanillaFilter?: true // 指定時は**効果の記述を持つ**スピリットのみ（vanillaFilter の裏。BS11-082ウィッグバインド＝「効果の記述を持つ相手のスピリットすべて」）
          lentOnly?: boolean // 仮想発生源（lendSelfThisTurn でこのターンだけ貸した効果）からのみ有効。**2026-08-24 追加**：データには書いてあったが型に無く、実装が読んでいなかった
...
export interface CardInstance {
    ...
    cantBlockThisTurn?: true // **このターンの間**ブロックできない（banBlockByCostsThisTurn。ターン終了でリセット。BS11-057バタホルン）
    // freeMagicAfterPaidMagic（BS11-X05）の消費記録。ターン番号とその中で使った回数
    freeMagicUsedTurn?: { turn: number; count: number }
    ...
    // **継続の自己コスト増減**（kind:"selfCostMod"。BS11-017ムシャツバメLv2-3＝『自分のアタックステップ』
...
export interface PendingChoice {
    ...
    trashSummonConfirm?: {
        // トラッシュにあるカードを**コストを支払わず**召喚するかの確認待ち（BS11-004）。
        // fushiSummon と同型で **action は解決しない**。維持コアはリザーブから置く（払えなければ召喚できない）
        pid: PlayerId
        cardId: string
...
export interface GameState {
    ...
    // トラッシュからの無償召喚（kind:"trashSummonOnOwnSummon"）の確認待ちの積み場所。
    // pendingReviveConfirms と同じく、アクションを解決しきった地点で1件ずつ pendingChoice にする
    pendingTrashSummons?: { pid: PlayerId; cardId: string; trashIndex: number }[]
...
export type TurnConstraintDef =
    ...
    | { type: "handLockExceptColorForPid"; pid: PlayerId; color: Color } // このターンの間、この pid は指定色以外の手札のカードを使えない（BS11-082）
