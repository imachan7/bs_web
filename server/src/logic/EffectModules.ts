...
export function resolveAction(
    ...
    // 分割済みアクションはハンドラ表へ委譲する（server/src/logic/actions/）。
    // 未移設のものは下の switch が処理する。移設完了時に switch を削除し、
    // ACTION_HANDLERS を ActionRegistry（全 type 網羅）として型検査する
    const ctx: ActionCtx = {
        state,
        owner,
        opp,
        ...
        chosenOption,
        chosenCardIndex,
        resolve: (next, selfOverride) =>
            resolveAction(
                state,
                owner,
                selfOverride === undefined ? self : selfOverride,
                next,
                targetInstanceId,
                sourceColor,
                sourceType,
    ...
    if (handler) {
        ;(handler as (c: ActionCtx, a: EffectAction) => void)(ctx, action)
        return
    }
