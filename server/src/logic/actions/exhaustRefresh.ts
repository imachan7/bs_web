...
import { KEYWORDS, cardNameContains, effectActiveAtLevel, effectiveBp, hasArmorAgainst, hasFullEffectImmunity, hasMagicImmunity, instColors, instHasColor, instHasCost, instIsCombined, isVanillaCard, matchesFamilyFilter, matchesTarget, spiritHasFamily, spiritHasKeyword, instMatchesCostFilter, bravesOf } from "../../../../shared/rules"
...
const refreshAllOwnHandler: ActionHandler<"refreshAllOwn"> = (ctx, action) => {
    const { state, owner, opp, self, sourceName, srcColors, srcType, destroyContext, targetInstanceId, chosenOption, chosenCardIndex } = ctx
        ...
        for (const s of player.field.spirits) {
            ...
            // exemptCombined（BS11-079リブートコード＝「合体スピリット以外のスピリットはアタックできない」）
            const exempt =
