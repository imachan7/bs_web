...
### いまの本線：期間つき効果の一覧は移行完了（2026-09-25）。M8 §A も 09-26 に移し終えた

[TIMED_EFFECTS.md](./docs/design/TIMED_EFFECTS.md) の一覧 `timedEffects` に全内容を移し終えた（同 §3.2）。M8 の残りは、移した旧 action のカードデータを `timedEffect` へ書き直す作業（支払い付きのムーンショウウオ・冥府へ続く魔門は `pay` の判定表を広げる必要がある）。
未着手の決定済み事項：②破壊直前の発生源を控える（ユーザー決定 a）。
回答待ち：④セイ・ドリガンの「このステップの最初に」の義務が消える条件／クロスシザースの「指定する」は必須か任意か／ブロック時効果と『ブロックされたとき』効果を同時発揮にしてターンプレイヤーに解決順を選ばせるか（今は決まった順。TIMING_CHART ＞３）。
