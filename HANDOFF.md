...
### M8 誘発効果の付与（進行中。ブランチ `chore/timed-grant-effect`）

`TimedContent` に `{ type: "grantTrigger"; trigger; action; battleRole? }`（期間 `turn`・`side:"own"`）。1体＝自分から選んで個体の `tempGrantedTriggers`、
`all: true`＝`timedRule`（後から出たスピリットにも付く＝ヤツノカンゾウだけ挙動が変わる）で `fireTrigger` が合流させる。旧 `grantEffectToTargetThisTurn`・`grantEffectToAllByKeywordThisTurn` を消す。
