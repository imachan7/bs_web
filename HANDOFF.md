...
### BS15 バッチ1・2（赤紫・緑白）と未実装節（§7）はすべて実装済み（2026-09-18。smoke part331〜345）

- 簡略化（card-notes 無し）：X01 Lv2-3 はスピリット状態のブレイヴも破壊候補に含む／016・X02 のコア除去は1体からまとめて取る／004 の Q3469 は未実装
- X04 の追加メインステップ中は、相手のプロボケイション確認を出さず、アタック強制でターン終了を止めない（その後にアタックステップが無いため）
- `coverage:effects` に `extraStepAfterAttackStep` の計測点は未追加（弾の統合時に回して確認する）
- **バッチ3（黄・青）の解釈は確定**（BS15_PLAN §2.5 F〜J、2026-09-18）。Sonnet 1体に委譲中。出力は `data/cards/BS15-yellow.json` / `BS15-blue.json`
