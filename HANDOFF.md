...
### M8 ⑥ キーワードを与える（ブランチ `feat/timed-keyword`。2026-09-24 確定スキーマ）

`grantKeyword{keyword, colors?}` → `timedEffect{content:[{type:"keyword",keyword,colors?}], duration:"turn", side:"own"}`（5か所。対象は旧と同じ `pickOwnKeywordTarget`、個体の `tempKeywords` に書く）。
BS03-105 暗礁海域 Lv2『自分のアタックステップ』は見出しの継続効果なので `keywordGrant{keyword:"jugeki", target:"ownAll", costFilter:{in:[2]}, phase:"attack", turn:"own"}` へ。旧 `grantKeyword`・`grantKeywordAll` は消す。
