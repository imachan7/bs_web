...
**BS12-004 ドラゴン・フェゼントの Lv2「自分のスピリット状態のブレイヴすべてをBP+3000」**：効果文に『』の見出しが無い＝Lv2 の間ずっと効く継続効果のはずだが、データはアタック時の誘発（`BS12-004-e2`）。ユーザーに確認してからオーラへ直す（2026-09-24 の「アタック時・ブロック時の BP+ はバトルの間」の変更からは外してある）。
