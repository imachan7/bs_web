...
**97枚すべての設計が終わっている（2026-09-08）。残りは実装だけ。**
...
| 器 | 先に作る | 後で使う | 注意 |
| :-- | :-- | :-- | :-- |
| W 相手による破壊限定 | 紫 010 | 緑 066 | |
| トラッシュ自己回収 | 紫 015 | 緑 077（ターン1回つき） | |
...
| 論点 | ぶつかる先 |
| :-- | :-- |
| 「疲労状態で召喚」で『疲労時』は誘発しない（「疲労する」と「疲労状態になる」は別） | **緑の器AB**（BS13-065 / X03） |
| 破壊時・バウンス時のコア移動は「取り除く」に該当しない | **緑の器AC**（BS13-065 Lv2 / X03 Lv3） |
| 「効果で手札が増える」にドローステップのドローは含まない（「ドロー+1」も非該当） | **黄の器Y**（BS13-063 Lv2） |
| 消滅待機中のシンボルが軽減に数えられている（**ギャップ確定・実測済み**） | 独立。直し方は小さい |
| 余分コストは軽減の**あと**に乗る／コスト固定は「後から発揮した方」が優先（実装は最小値） | コスト周りに触るとき |
| 解決の途中で破壊状態が解除されたら、以降の破壊誘発は処理しない | 解決順を選ばせる話。**ユーザー確認から** |
