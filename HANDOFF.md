...
### M8 トリガー抑止（進行中。ブランチ `chore/timed-suppress-trigger`・挙動は変えない）

`TimedContent` に `{ type: "suppressTrigger"; trigger: TriggerEvent }`（期間 `turn`）。1体＝既存の1体経路で個体の `suppressedTriggersThisTurn` へ、
`all: true`＝`state.triggerSuppressionThisTurn`（陣営は `side`）。旧 `markSuppressTriggerThisTurn`・`suppressTriggerThisTurn`（2か所）を移して消す。
