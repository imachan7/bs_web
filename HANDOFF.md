...
**「フィールドに残る」で戻ったスピリット自身の『破壊時』効果は発揮するか。**
現状の `reviveOnDestroy` は**発揮しない**（`tryReviveOnDestroy` が `onDestroy` の発火より手前で返る）。
一方 §1 #20 では「破壊待機を経て、破壊で誘発された効果をすべて処理してから」と確認しているので、
発揮するのが正である可能性が高い。**直し方は小さいが、既存カード10枚以上が使う共有部品**なので
答えが出るまで触らない。詳細と実測は [BS13_PLAN.md](./docs/design/BS13_PLAN.md) §8.3。
