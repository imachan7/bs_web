...
### M8 疲労状態でもブロック（進行中。ブランチ `chore/timed-block-while-rested`・挙動は変えない）

`TimedContent` に `{ type: "canBlockWhileRested" }`（期間 `turn`）。すべて＝`all`・`side:"own"`・`filter.family` の `timedRule`、
1体＝`count:1`・`side:"own"`・`filter.color` で個体の印 `canBlockWhileRestedThisTurn`。ターン制約 `canBlockWhileRestedThisTurn` は消す。
