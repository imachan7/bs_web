...
**次の一手: バッチ6（青13枚）を入れる。** バッチ0〜5 は投入済み（76/91枚）。
バッチ5で確定した規則は手順書へ移した（[BS12_PLAN.md](./docs/design/BS12_PLAN.md) §6）。

**残る青13枚・多色2枚は設計が全部済んでいる** → [BS12_PLAN.md](./docs/design/BS12_PLAN.md)
§1 の #13〜#18（確定した解釈）と §7（青）／§8（多色）。**そのまま順に委譲できる。**
