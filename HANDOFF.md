...
- **既存バグ3件は別PRで修正**：破壊時効果の二重発火（#72・part341）／バースト効果の装甲すり抜け（#73・part342）はマージ済み。
  メインからのターン終了でアタックステップを飛ばす件（#74・part343）は**マージ待ち**。#74 のマージ後に BS15 へ取り込み、
  **079 プロボケイションの確認をメインから直接ターン終了する経路にも挟む**（今は `nextPhase` にだけある）
- **ブロッカーの疲労はブロック宣言時に移した（PR #71 でマージ済み。smoke part340）**。複数体ブロックでバトルしない側も疲労する。【暴風】をブロック時へ差し替える継続付与（大風車の丘 Lv2）はバトル解決時のまま
- **未実装節（BS15_PLAN §7）**：011（手札から使うフラッシュ・part334）／015（紫マジックの色を無いものとして扱う・part335）／
  064 Lv2（【不死】の無償召喚）と Lv1-2 の手札破棄を持ち主が選ぶ形に修正（part336）は**実装済み（2026-09-17）**。
  【不死】の確認は、魔門で無償召喚できるときだけ「コストを支払って召喚する／魔門を疲労させて無償で召喚する」の2択、それ以外は従来どおり「召喚する」。
  **残りは X04 Lv2（§7.1）と、079 プロボケイションの確認をメインから直接ターン終了する経路にも挟むこと**（#74 はマージ済み・BS15 へは未取り込み）
