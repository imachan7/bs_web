...
2. **R3 の続き**（いまここ）：済み＝removal → `brave.ts`・`revive.ts`（#156）、`shared/rules.ts` → `shared/rules/` 8本（#157）、GameEngine → `choice.ts`・`battleResolve.ts`。
   残り＝`validate:size` の据え置き一覧（`scripts/check-file-size.ts`）の7本：EffectModules・cores・destroy・battleFlow・triggers・型2本。
