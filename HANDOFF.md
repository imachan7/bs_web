...
### M8 ⑨ シンボル・コスト（ブランチ `feat/timed-symbol-cost`。2026-09-24 確定スキーマ）

内容を4つ足す：`{type:"symbolAdd"}`（持つシンボルと同じ色を1つ追加。ダブルハート）／`{type:"symbolSet"; color; count}`（シンボルを◯色◯つとして扱う。ゴエモン）／
`{type:"symbolLoss"; color?}`（指定色のシンボルを1つ失う。color を省くと使う人が選ぶ。バキュームシンボル）／`{type:"cost"; amount}`（コストを増減。グロウアップ）。
対象の決め方は旧と同じ。**バキュームシンボル（相手のスピリットすべて）は `all: true` で、後から出たスピリットにも効く**（「すべて」の一般則。Lv と同じく `refreshLevelAsOverrides` で未書き込みの個体に書く）。
