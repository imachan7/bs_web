...
**M8「Lv を〜として扱う」（§A の4種・11か所。2026-09-24 調査のみ・未着手）**
...
- 1体を指定する2種（`levelOverrideTarget` 7か所＝候補の先頭を自動選択、`levelUpThisTurn` 2か所＝`pickSingleTarget`・実効BP最大）は `timedEffect` の内容 `level` で移せる。対象の決め方が2通りあるので、それぞれ旧と同じにする
