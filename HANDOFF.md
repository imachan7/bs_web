...
→ [BS12_PLAN.md](./docs/design/BS12_PLAN.md)。§1 に確定済みの解釈18件、§2 に新しく要る器の一覧、§4 にバッチ割り

**次の一手: バッチ5（黄13枚）を入れる。** バッチ0〜4 は投入済み（63/91枚）。
バッチ4で確定した規則は手順書へ移した（[BS12_PLAN.md](./docs/design/BS12_PLAN.md) §5）。

**残る黄13枚・青13枚・多色2枚は設計が全部済んでいる** → [BS12_PLAN.md](./docs/design/BS12_PLAN.md)
§1 の #9〜#18（確定した解釈）と §6（黄）／§7（青）／§8（多色）。**そのまま順に委譲できる。**
