...
**「フィールドに残る」で戻ったスピリット自身の『破壊時』効果は発揮する**（2026-09-08 ユーザー確認）。
規則は [TIMING_CHART.md](./docs/design/TIMING_CHART.md) へ移した。**現状の実装はこれに反している（既知のバグ）。**

直し方: 復活の適用を `tryReviveOnDestroy` の位置から `commitPendingDestruction`
（トラッシュ行きの確定地点。中断・再開の経路4つがすべてここを通る）へ移す。
**未着手。着手前に次の2つを確認すること:**

1. **`destroySpirit` の戻り値の意味が変わる**（復活しても「破壊された」＝ true）。
   `destroy.ts` の `destroyed++` のような**「破壊した体数」の数え方**に影響する
2. **既存カード10枚以上の挙動が変わる**（チャガマル／紫水晶の森／鏡の回廊／無法者の荒野／深緑の樹海／
   子供部屋 午前0時／BS05氷の魔女ヘル／BS05プリンセス・スノーホワイト／BS12-068光の聖剣／
   BS12-X05戦神乙女ヴィエルジェ）。**強くなる方向**なので §3 の表に足すこと
