...
**バッチ5（黄13枚）と バッチ6（青13枚）の設計は済んでいる** → [BS12_PLAN.md](./docs/design/BS12_PLAN.md)
§1 の #9〜#16（確定した解釈）と §6（黄の器）／§7（青の器）。白バッチが終わったら順にそのまま委譲できる。
残るは多色2枚（040 / X008）だけで、これは全バッチの後。
