...
### M2 `if`：解釈一覧を出してユーザーの答え待ち（2026-09-27）

[IF_UNIFY.md](./docs/design/IF_UNIFY.md) §3 の Q1〜Q5 に答えをもらってから器を作る。**次のセッションはまずここを聞く**。
Q1（BS14-X01 の条件が召喚まで掛かるか）は答えによって挙動が変わる。器の形（`if`・`cond`・`lastCount`・`sameAsLast`）は §1 の案
