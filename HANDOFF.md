...
### いまの本線：期間つき効果の一覧は移行完了（2026-09-25）。次は M8 の残りへ戻る

[TIMED_EFFECTS.md](./docs/design/TIMED_EFFECTS.md) の一覧 `timedEffects` に全内容を移し終えた（同 §3.2）。止めていた M8 の残り（REFACTOR_PLAN §2.2）を、一覧へ書く形で再開する。
未着手の決定済み事項：②破壊直前の発生源を控える（ユーザー決定 a）③アブソリュートストライクを `timedEffect` で書く（`count:"any"` のトグル選択＋マジックの condition に名前条件）。
回答待ち：④セイ・ドリガンの「このステップの最初に」の義務が消える条件／クロスシザースの「指定する」は必須か任意か。
