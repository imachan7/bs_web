...
### 簡略化5件を原作どおりにする（2026-09-07 ユーザー指示。完了したら手順書へ移してここから消す）

| # | カード | 直す内容 |
| :-- | :-- | :-- |
| 1 | BS11-036 冥土の魔女ヘレン | `handMagicToTegamotoDraw` に上限の軸を足し、**3枚まで**にする。いまは「好きなだけ」でドローが青天井 |
| 2 | BS11-042 海賊ラッコルセア | 【粉砕】で**スピリットカードが1枚以上置かれたとき**だけ発火させる。`triggered.condition` には `lastFunsaiHasSpirit` が既にあるので、`fieldEvent` 側にも同じ軸を足す |
| 3 | BS12-078 カシオペアシール | デッキ横のコア5個を実際に持つ（`PlayerState` に枚数を足し、自分のエンドステップに1個ずつ戻す）。**画面にも出す** |
| 4 | BS08-062 超時空重力炉 | 「このネクサスのシンボルを白3つに**する**」＝置換。`reductionGrant` に置換の軸を足す（いまは白2つの**追加**） |
| 5 | BS04-081 強者統べる大地 | **実装は既に正しい**（選択式になっている）。`card-notes.json` の項目を削除し、`type.ts` の「BP最大＝指定の決定的簡略化」という古いコメントを実態に合わせる |

⚠️ **#3 と #4 は、現状のエンジンでは挙動が変わらない**（2026-09-07 調査）。
ボイドは**残量を持たない無限の供給源**として実装されているので、デッキ横へ5個移しても観測できる差が無い。
#4 も、このネクサスの素のシンボルが白1つで固定なので「白1つ＋白2つ追加」と「白3つに置換」は同値。
**買えるのは「将来ボイドを有限にしたとき／ネクサスのシンボルを変えるカードが出たときの正しさ」と画面表示だけ。**
