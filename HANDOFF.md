...
### M8 次の一手：見出しの無い／ステップ見出しの BP+ をオーラへ（#108 のマージ後に最新の main から）

- 『自分のアタックステップ』見出しの BP+（BS13-061・BS16-007・BS16-024）は、ステップ開始時の誘発（`kind:"step"`）から「そのステップの間ずっと効くオーラ」（`aura` ＋ `phaseTurn: { phase: "attack", turn: "own" }`）へ書き直す。発揮し続ける効果なので古代闘技場は判定のたびに効く
- BS12-004 ドラゴン・フェゼントの Lv2「自分のスピリット状態のブレイヴすべてをBP+3000」は見出しの無い継続効果（2026-09-24 ユーザー確認）。アタック時の誘発（`BS12-004-e2`）から Lv2 のオーラへ書き直す
