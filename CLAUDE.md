...
- **色ごとに1体を立てない。2色で1体にする**（2026-09-12 ユーザー指示）。コールドスタートは
  **エージェント数に比例**し、2色目の限界費用はほぼ実装ぶんだけ。4体→2体で30〜50万トークン浮く。
  カードデータが色別ファイルに分かれていれば編集は衝突しない
  （BS14 を4色並列で回したとき、5時間制限の手前まで使い切った）
