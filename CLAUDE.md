...
- **委譲時のルール**:
  ...
  - **新しい type／kind を確定させる前に、既存の器の組み合わせで書けないかを確かめる**（2026-09-22 ユーザー合意）。
    「その後」は `sequence`、絞り込みは `filter`、「〜1体につき」は `countCounter` で書ける。
    書けないときだけ新設し、**1枚専用の名前（`millThenCoreIfBurst` のような動詞の連結）にしない**。
    1枚ずつ専用の type を確定させてきた結果、328種のうち180種がカード1枚でしか使われていない（[REFACTOR_PLAN.md](./docs/design/REFACTOR_PLAN.md) §2）
