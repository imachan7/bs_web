...
- **ヘルパーの置き場は [docs/CODEMAP.md](./docs/CODEMAP.md) を名前で grep させる**（`grep -n "matchesFamilyFilter" docs/CODEMAP.md`。丸読みは約2万トークンなのでしない）。
  export の名前・種類・直前のコメント1行がファイルごとに並ぶ。export を足したり、その直前のコメントを変えたら `npm run codemap` で作り直してコミットする（CI の `codemap:check` が古い索引を落とす）
