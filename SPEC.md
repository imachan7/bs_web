...
- 構造化の進捗: キーワード7枚＋赤・紫バッチ21枚（全文10・部分11。スキップ9枚の未対応概念は
  ...
  deployNexus all を新設、BS03-073/087/097/100/144/145/148/X12）の計 **78枚** / 効果文持ち128枚。
- **残り50枚は表示のみで一旦確定（2026-07-19）**。主な未対応概念: 誘発効果の任意コスト支払い
  （「コア◯個を置くことで」系）・効果無効化・「手札にカードが加えられたとき」イベント・
  粉砕の実破棄枚数参照・player-choice可変数（好きなだけ破棄等）・相対レベル操作・破壊耐性。
  BS02 同様、再開時はこのリストから優先度をつけて拡張する
