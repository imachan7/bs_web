...
- 構造化の進捗: キーワード7枚＋赤・紫バッチ21枚（全文10・部分11。スキップ9枚の未対応概念は
  ...
  自分側のみに簡略化）の計 **118枚** / 効果文持ち128枚。
- **残り10枚は表示のみ（2026-07-24 時点）**。いずれも新概念が必要な単発:
  効果無効化（ウッド・ゴレム＝相手ネクサスLv2効果無効）・手札上限の強制破棄（オリバー）・
  手元カード参照（エクリア）・ライフ被弾の条件付き無効（フレイア）・
  バトル参加者の入れ替え（テレポートチェンジ）・疲労数同期（セイムタイアード）・
  疲労コストのBP移譲（ユナイテッドパワー）・マジック無償化（バロッサ）・
  極光の大地（相手効果によるコア移動の検知）・トランプの王国（疲労免疫）。
