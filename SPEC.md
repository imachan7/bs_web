...
- **クライアント連動バッチ（2026-07-25）**: `reductionGrant` に familyFilter（OR配列。対象は手札カードの
  ...
  **BS04 構造化は effects付き 73/97枚（効果文持ち）に到達。残る未構造化24枚は下記の末尾**
- **未構造化で残る24枚（表示のみ。真に新概念が必要）**:
  ...
  - 「自分のスピリットが召喚されたとき」の FieldEvent 不在＋動的BP参照: 077 e2・083 e2
    （実装すれば2枚とも全文化できる有力候補）
  - 対象/名前フィルタ・同系統全破壊など単発概念: 024（ターン最初のアタック検知）・027（一度に5枚以上破棄検知）・
    037・042・054・081・090・101・110・112
