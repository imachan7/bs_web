...
| R7 | クライアントとサーバーの判定を `shared/rules.ts` に一本化する（まずコスト支払いから） | C | 大 | 調査から。`audit:parity` で確かめる |

**順番**（2026-09-26 見直し・ユーザー了承）
1. R2 と `validate:size`（§3 の再発防止）。`validate:size` は、今ある7本を例外の一覧に入れてから始める
2. R3 の続き（removal → GameEngine → shared/rules、あわせて cores・destroy・battleFlow）。分割1つごとに R1 の手順書の該当行を書く
3. **一度 BS16 の黄・青（バッチ3）に戻り、§0 と同じ形で実装役の呼び出し数を測る**（リファクタリングが「探す手間」に効いたかを確かめる）
4. R5 の残り（§2.2）と R6・R7 は、3 の測定結果を見て優先順位を決め直す

**09-22 の見直し前に「最初にやる」としていた R1・R2 が手つかずのまま R5 を進めていた。** 目的（§0）に一番直接効くのは R1・R2 なので、順番を戻した。
...
| 3 | M1 `pay` | 18種・延べ44枚 | `pay { cost, then }` と「完全に解決できる」の判定 | **済み（09-27、#172〜#175 と `feat/pay-life-cost`）**。then が cost の結果を見る4枚（BS13-024・058・060・BS15-067）と costReturnSelfToHand の2枚は旧 type のまま（PAY_UNIFY §5）。量が支払いの結果で決まる6種は M2 と一緒に |
...
| 6 | M8 期間つき継続効果 | 88種 | `timedEffect`（期間つきで置く器）と一覧 `timedEffects`（TIMED_EFFECTS.md） | **09-26 に一区切り**。置き場は一覧1つにまとめ、`turnConstraints` と個体・バトルの印は消した。期間つき効果を置くだけの旧 action 19種は、対象の選び方・支払いを持ち記録は一覧に書くだけの入口として残す（書き直すとカードごとに対象の選び方が変わるので、周りを触るときに1枚ずつ確かめる） |
| 7 | コアの統合（置く・取り除く） | 旧55種 | `placeCores`・`removeCores`（CORE_UNIFY_PLACE.md・CORE_UNIFY_REMOVE.md §3） | **09-26 に一区切り**（#165〜#169）。旧 type で残るのは、コスト付き・条件付き・直前の結果を使うもの・入れ替え／移動の約10種（pay・if・移動の器がそろったら移す） |

1〜2 は確認なしで進められる。3・4 は着手前に下の確認を取る。

**M8 から分かったこと**（09-26）
- 旧 type のカードデータを新しい器で書き直すと、**対象の選び方がカードごとに変わる**（耐性を見るか、自分と相手のどちらから選ぶか、など）。旧 type を「記録を書くだけの入口」として残すのも終わり方の1つとして認める
- 移すときに規則の食い違いが見つかる（シンボルの固定、BP の書き換え、ボイドとトラッシュ、「使えない」条件）。一方で、移行で不具合を入れることもある（「すべて」がネクサスに当たった）。**移したら、実際のカードデータで効果を解決するテストを1本足す**
- 器の中に内容ごとの分岐（`place〜` 関数）が増えていく。器が振り分けの switch にならないよう、新しい内容は既存の分岐に乗るかを先に確かめる
...
| `EffectModules.ts` | 4239 | 【転召】（`tenshoSpecOf`〜`applyTenshoSubstitute*`）、【粉砕】【呪撃】【暴風】【強襲】など**キーワードごとの判定**、デッキ破棄（`millDeck`・破棄無効）、疲労・回復（`exhaustSpirit`・`refreshSpirit`） → `keywords/tensho.ts`・`keywords/<キーワード>.ts`・`zones/mill.ts`・`state/exhaust.ts`（済み）。09-26 に継続効果の再計算 → `state/continuous.ts`、対象の選び方と選択待ち → `targeting.ts`、バースト → `keywords/burst.ts`、召喚時の誘発と無償召喚 → `summon.ts`（1393行） |
