...
| R3 | **責務単位の分割**（§3）。`handDeck.ts` の6分割を最初に、名前と中身が食い違っているファイルを概念ごとに分ける | E・A | 中（移すだけ） | handDeck・EffectModules（キーワード・デッキ破棄・疲労）は分割済み（09-23）。次は triggers（マジック） |
...
**分けるかどうかは、カードのテキストで機械的に決める**：2つの操作の**どちらか一方にだけ反応するカードかルールがある**
（「〜されたとき」の誘発／「〜されない」の耐性／破壊待機・バースト条件のようなルール上の処理の違い）なら別のアクションにする。
件数が0ならまとめ、はっきり多ければ分ける。**ユーザーに確認するのは件数が少ない境界の語だけ**。
