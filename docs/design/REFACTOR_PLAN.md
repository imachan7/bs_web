...
| 6 | M8 期間つき継続効果（既存の内容あり） | 51種・延べ95枚 | 器「継続効果を期間つきで置く」＋期間の値。内容は既存の継続効果の語彙をそのまま使う | **器 `timedEffect` を作り、「アタック／ブロックできない」を個体に付ける3種（4か所）を移行済み（09-24）**。全体ルール型（`banActByCostThisTurn` など turnConstraints に積むもの）は、後から出たスピリットにも効くので対象の軸を別に決める |
