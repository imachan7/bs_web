...
> ## ✅ 2026-08-07：この手引きが対象にしていた36枚は片付いた
>
> ベースラインは **17件 → 5件**まで縮んだ。残る5件は**すべて「実装しないと決めたもの」**で、
> 理由は `data/card-notes.json` に書いてある（BS02-083 鏡の回廊 Lv2／BS03-147 ゴーレムクラフト メイン／
> BS05-038 シーサーズ Lv2／BS05-079 スリーカード。BS04-088 栄光の表彰台 Lv1 は 2026-08-24 に解消）。
> **§5 の表はもう作業リストではない**。以降は「この効果文をどの器で書いたか」を引くための履歴として読む。
>
> このバッチで足した器の一覧は [SPEC.md](../../SPEC.md) 2章「効果実装漏れの解消で足した器」にある。
> 新しい効果を書くときは、まず §4 の早見表と SPEC.md のその表を見ること。

このファイルは「次に何を、どの順で、どこを触って書くか」を1枚ずつピン留めしたもの。
§1（1枚を実装する手順）・§2（踏みやすい罠）・§4（早見表）・§6（テストの書き方）は
**新しいカードを実装するときにもそのまま使える**ので残してある。
...
## 5. 36枚の分類と方針（**2026-08-07 に全件処理済み。以下は履歴**）

> A〜C はすべて実装した。D は `data/card-notes.json` に理由を書いて見送った。
> 各行の「足すもの」欄は**実際に足した器と一致しないことがある**（実装時に方針を変えた箇所がある）。
> 正確な現状は SPEC.md 2章の表と `git log --grep` で引くこと。
...
| BS04-088 | 栄光の表彰台 Lv1 | 解消済み（2026-08-24）。コア払いとデッキ払いを配置時に選ぶ |
